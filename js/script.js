...
link.addEventListener("click", function(a) {
        a.preventDefault(), popup.classList.add("modal-show"), storage_name ? (nickname.value = storage_name, storage_email ? (email.value = storage_email, letter.focus()) : email.focus()) : nickname.focus()
        ...
        a.preventDefault(), map_modal.classList.add("modal-show")
    }), map_close.addEventListener("click", function(a) {
        a.preventDefault(), map_modal.classList.remove("modal-show")
    }),
    function(a, b) {
        var c = {
            _modules: {},
            require: function(a, b) {
                function g(e) {
                    return function(f) {
                        d[e] = f, ++c == a.length && b.apply(null, d)
                    }
                }
                a = [].concat.call([], a);
                for (var c = 0, d = [], e = 0, f = a.length; f > e; e++) this._modules[a[e]] && this._modules[a[e]](g(e));
                return this
            },
            define: function(a, b, c) {
                function f(a) {
                    ...
                    var f = [],
                        g = 0;
                    if (b.length)
                        for (var h = 0; h < b.length; h++) d.require(b[h], i(h));
                    else j()
                }
                var e, d = this;
                return b = [].concat.call([], b), this._modules[a] = function(a) {
                    e ? a(e) : f(a)
                }, this
            }
        };
        c.define("main", ["map-data", "params"], function(a, b, d) {
            function h() {
                ...
                return a.setAttribute("id", "ymaps" + d.tid), a.style.display = "block", a.style.width = d.containerSize[0], a.style.height = d.containerSize[1], a
            }

            function i(a) {
                for (; a;) {
                    if (a.parentNode === document.body) return !0;
                    a = a.parentNode
                }
                return !1
            }
            ...
            if (b.maps && b.maps.length && d.testUrl) {
                for (var f, e = document.getElementsByTagName("script"), g = e.length - 1; g > -1; g--)
                    if (f = e[g], -1 !== f.src.indexOf(d.testUrl) && !f.ctorInited) {
                        f.ctorInited = !0;
                        break
                    } f && c.require(["ymaps", "create-map"], function(a, b) {
                    var c, e = h();
                    if (d.elementId) {
                        if (c = document.getElementById(d.elementId), !c) return void j("DOMElement #" + d.elementId + " not found.");
                        c.appendChild(e)
                    } else {
                        if (!document.documentElement.contains(f)) return void j("Script element was removed from document.");
                        i(f) ? f.parentNode.insertBefore(e, f) : document.body.appendChild(e)
                    }
                    b(e), f.parentNode && f.parentNode.removeChild(f)
                }), a({})
            }
            ...
            e.testUrl = f && f[1], e.containerSize = [g(e.size[0]), g(e.size[1])], e.isEnterprise = c.originalUrl.indexOf("//enterprise") > -1, e.lang = (e.lang || d.maps[0] && d.maps[0].state && d.maps[0].state.lang || "ru_RU").replace("-", "_"), e.tid = String(Number(new Date)) + String(Math.round(1e6 * Math.random())), e.ns = [c.namespace, e.lang, e.key.replace(/\W/g, ""), e.apikey.replace(/\W/g, ""), e.isEnterprise ? "_ntrp" : ""].join("__"), a(e)
            ...
            a(function(a, b) {
                var c = document.getElementsByTagName("head")[0],
                    ...
                    e = function() {
                        b && (b(), c.removeChild(d)), b = null
                    };
                return d.charset = "utf-8", d.src = a, c.insertBefore(d, c.firstChild), d.onreadystatechange = function() {
                    ("complete" == this.readyState || "loaded" == this.readyState) && e()
                }, d.onload = e, d.src = a, d
            })
            ...
            var e = c.ns,
                f = window[e],
                g = e + "loader";
            if (f && !window[g]) f.ready(l);
            else if (window[g]) window[g].queue.push(l);
            else {
                var h = "fid" + c.tid,
                    i = b.host;
                ...
                c.key && (j += "&key=" + c.key), c.apikey && (j += "&apikey=" + c.apikey), d(j + "&onload=" + h), window[h] = function(a) {
                    a.ready(function() {
                        function a() {
                            var a = window[g];
                            try {
                                delete window[h], delete window[g]
                            } catch (b) {
                                window[h] = window[g] = null
                            }
                            for (var c = 0, d = a.queue.length; d > c; c++) a.queue[c]()
                        }
                        a()
                    })
                }, window[g] = {
            ...
            a(function(a) {
                function c() {
                    for (var f = document.documentElement, g = a.container.isFullscreen() ? [f.clientWidth, f.clientHeight] : a.container.getSize(), h = b.minContainerSize, i = "0,0" == g.toString(), j = g[0] < h[0], k = !i && (j || g[1] < h[1]), l = ["rulerControl", "routeEditor", "searchControl", "trafficControl", "geolocationControl"], m = 0, n = l.length; n > m; m++) a.controls.get(l[m]).options.set("visible", !k);
                    (a.state.get("narrowMode") || !a.options.get("suppressMapOpenBlock", !1)) && (a.state.get("narrowMode") !== j ? j ? d.show(a) : d.hide(a) : j && d.onResize(a)), a.state.get("compactMode") !== k && (k ? (a.controls.remove("typeSelector"), a.controls.get("zoomControl").options.set({
                        position: {
                            top: 10,
                            left: 10
                        },
                        size: "small"
                    })) : (a.controls.add("typeSelector"), a.controls.get("zoomControl").options.unset(["position", "size"]))), i ? e || (e = window.setInterval(c, 200)) : e && (window.clearInterval(e), e = 0), a.state.set({
                        compactMode: k,
                        narrowMode: j
                    })
                }
                a.container.events.add(["fullscreenenter", "fullscreenexit"], c), window.attachEvent ? window.attachEvent("onresize", c) : window.addEventListener("resize", c), c()
            })
            ...
            a(function(a) {
                ...
                var b = f.templateLayoutFactory.createClass("{{ properties.iconContent }}", {
                        build: function() {
                            ...
                            if (a && String(a).length > 2) {
                                ...
                                c.style.fontSize = "9px", c.style.lineHeight = "16px", c.style.display = "block"
                            }
                        }
                    }),
                    h = {
                ...
                for (var k in j) j.hasOwnProperty(k) && f.option.presetStorage.add(k, j[k]);
                ...
                for (var l = g.geoObjects.features, m = f.map.associate.serviceGeoObjects.get(i), n = 0, o = l.length; o > n; n++) {
                    var q, p = l[n];
                    p.options && (q = r(p.options.preset || "")), m.add(new f.GeoObject({
                        geometry: p.geometry,
            ...
            function n(a) {
                function b(a) {
                    var b = a.getElement();
                    b && b.firstChild && b.parentNode && (b.firstChild.style.width = "100%", b.firstChild.firstChild && (b.firstChild.firstChild.style.padding = "0px"))
                }
                m = "cnst_" + +new Date, l = new d.control.Button({
                    data: {
                ...
                }), l.getLayoutSync() ? b(l.getLayoutSync()) : l.getLayout().then(b), a.panes.get("copyrights").getElement().style.marginBottom = "29px", o(a)
            }
            ...
            function o(a) {
                function b(b) {
                    ...
                    d[0] < 175 ? (c.style.paddingLeft = "0px", c.style.backgroundImage = null) : (c.style.paddingLeft = "18px", c.style.backgroundImage = "url(" + i + ")")
                }
            ...
            a({
                show: function(a) {
                    ...
                    }), n(a), p(a), l.events.add("click", function() {
                        window.open([b.mapHost, "?um=", c.sourceType, ":", c.sid, "&source=constructor"].join("")), f("mapsButton-constructor.smallMap")
                    })
                },
                onResize: function(a) {
                    o(a)
                ...
                hide: function(a) {
                    l && a.controls.remove(l), q(a), a.panes.get("copyrights").getElement().style.marginBottom = "0px", a.options.unset(["copyrightLogoVisible", "suppressMapOpenBlock"]), k && (k.removeAll(), k = null)
                }
            })
